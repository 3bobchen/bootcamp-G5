...
export default function GarmentUpdateForm(props) {
  const {
    id: idProp,
    garment,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = garmentRecord
      ? { ...initialValues, ...garmentRecord }
      : initialValues;
    setType(cleanValues.type);
    setColour(cleanValues.colour);
    setErrors({});
  };
  const [garmentRecord, setGarmentRecord] = React.useState(garment);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(Garment, idProp) : garment;
      setGarmentRecord(record);
    };
    queryData();
  }, [idProp, garment]);
  React.useEffect(resetStateValues, [garmentRecord]);
  const validations = {
    type: [],
    colour: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Garment.copyOf(garmentRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "GarmentUpdateForm")}
      {...rest}
    >
      <SelectField
        label="Type"
